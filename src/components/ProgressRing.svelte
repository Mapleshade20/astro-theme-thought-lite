...
<div class="ring-container" data-status={status} data-theme={theme} style="width: {size}px; height: {size}px;">
	<svg width={size} height={size} viewBox="0 0 {size} {size}" style="overflow: visible;">
		...
		<!-- Ripple circles (only visible for inProgress status) -->
		{#if status === "inProgress"}
			<circle class="ripple-circle" cx={size / 2} cy={size / 2} r={rippleRadius} stroke-width={strokeWidth * 0.3} />
			<circle class="ripple-circle" cx={size / 2} cy={size / 2} r={rippleRadius} stroke-width={strokeWidth * 0.3} />
			<circle class="ripple-circle" cx={size / 2} cy={size / 2} r={rippleRadius} stroke-width={strokeWidth * 0.3} />
		{/if}

		<!-- Progress ring -->
		<circle class="progress-ring status-{status}" cx={size / 2} cy={size / 2} r={radius} stroke-width={strokeWidth} stroke-dasharray={circumference} stroke-dashoffset={offset} stroke-linecap="round" />
	</svg>
